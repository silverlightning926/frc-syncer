...
from services.db_service import get_event_keys_for_year, get_tba_page_etag, upsert_event_rankings
from python_models.tba_page_etag import TBAPageEtag
from services.db_service import upsert_tba_page_etag
...
def process_event_rankings_response(response):
    if response.status_code == 304:
        ...
        print(
            f"Event Rankings: Failed to fetch event rankings. Status Code: {response.status_code}"
        )
        return None
...
def upsert_event_rankings_data(event_key, rankings, response, year: int):
    if rankings:
        ...
        print(f"Event Rankings ({event_key}): Fetched {len(rankings)} rankings.")
