...
from services.db_service import (get_event_keys, get_tba_event_teams_page_etag,
                                 upsert_event_teams, upsert_tba_page_etag)
...
def process_event_teams_response(response):
    if response.status_code == 304:
        ...
        print(f"Event Teams: Failed to fetch event teams. Status Code: {
